...
def parse(flow: list) -> list:
    ...
    for item in flow:
        ...
        if item["type"] == "image":
            img_node = create_node(node_type="image", src=item["src"], page=page_num)
            current_entry["children"].append(img_node)
            continue

        # 4. Detect Title (The very first text block after the date)
        if not current_entry["meta"]["has_title"] and len(clean_text) > 2:
            title_node = create_node(node_type="heading", text=raw_text, page=page_num)
            current_entry["children"].append(title_node)
            current_entry["meta"]["has_title"] = True
            continue

        # 5. Detect Quote (The first text block after the title)
        ...
        if clean_text.lower().startswith("just for today:"):
            aff_node = create_node(node_type="affirmation", text=raw_text, page=page_num)
        ...
        if len(clean_text) > 3:
            para_node = create_node(node_type="paragraph", text=raw_text, page=page_num)
